...
| 层级 | 适合验证的改动 | 代表文件 | 预期证据 |
|---|---|---|---|
| 契约与单元 | 单个 schema、变换、解析器、指标或 checkpoint 规则 | `test_config.py`、`test_voc.py`、`test_transforms.py`、`test_metrics.py`、`test_checkpoint.py` | 快速验证一个边界，产物只存在于临时目录 |
...
| 评估与推理 | AP 契约、错误分析、运行比较、报告或预测 | `test_evaluation.py`、`test_errors.py`、`test_comparison.py`、`test_inference.py` | 临时目录中的确定性 JSON、CSV、checkpoint 和图片产物 |
| 端到端 | 改动跨越 CLI、数据、训练、评估与预测边界 | `test_end_to_end.py`、`test_cli.py` | 一条完整离线合成流程，以及命令行解析器的实际行为 |
| 发布与示例 | 文档、package metadata、脚本或可运行示例 | `test_documentation.py`、`test_examples.py`、`test_packaging.py`、`test_scripts.py`、`test_download_data.py` | 有效链接和命令、声明的 package 文件、可离线执行的帮助命令，以及脚本产物契约 |
...
聚焦测试通过只代表选中的边界通过，不代表其他层或完整流程已经通过。`test_model_smoke.py` 会构造真实 torchvision 检测器，因此可能比 fake-detector 单元测试慢，但不会下载权重。

## 完整验证

提交跨边界改动前，运行完整套件，并把弃用警告提升为错误：

```bash
uv run --no-sync pytest -W error::DeprecationWarning
```

然后执行[参与贡献](../CONTRIBUTING.zh-CN.md)列出的静态检查。完整测试应把生成证据留在 pytest 临时目录，不改动仓库的数据或产物目录。

## 证据边界

测试通过证明的是被覆盖的软件契约，其中包括一条离线合成端到端路径。它不能证明检测器收敛，不能生成有界学习成绩，也不能建立完整 VOC 成绩。有界学习运行需要在 pytest 之外使用 `configs/learning_minimal.yaml`；独立的[完整 VOC 实测运行](../docs/recorded-run/README.zh-CN.md)使用官方准备数据与真实执行产物，测试不会重新生成它。
