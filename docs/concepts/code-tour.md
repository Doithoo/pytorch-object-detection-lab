...
Run `uv run detect show-config --config configs/learning_minimal.yaml` to exercise only this first boundary. Expected output is resolved YAML plus `sources`; no data/model/network/artifact work occurs.
...
`detection_collate` deliberately returns two lists. It does not pad or stack variable-sized images. When labels, difficult flags, dimensions, or geometry look wrong, debug this boundary before the model.
...
`uv run detect list-models` and `uv run detect model-info fasterrcnn_resnet50_fpn` inspect this boundary without construction. `examples/03_model_contract.py` crosses construction and both model modes with synthetic input, but proves no learned quality.

## Training and checkpoint boundary
...
Use `tests/test_end_to_end.py` for an executable package path and focused tests for the boundary changed. A parser unit test does not prove model integration; a dry run does not prove learning; a bounded run does not prove full VOC quality. The [recorded run](../recorded-run/README.md) is separate execution evidence; `configs/reference_fasterrcnn.yaml` alone remains only a recipe.
