...
XML 根节点必须提供非空 `filename`、正整数 `size/width` 和 `size/height`，并可包含零个或多个 `object`。每个目标需要受支持的 `name`，以及含有限数值 `xmin`、`ymin`、`xmax`、`ymax` 的 `bndbox`。可选的 `difficult` 默认为 `0`，若存在只能是 `0` 或 `1`。文件名和解码图像尺寸必须与 XML 一致。

支持的目标名称按标签顺序为：`aeroplane`、`bicycle`、`bird`、`boat`、`bottle`、`bus`、`car`、`cat`、`chair`、`cow`、`diningtable`、`dog`、`horse`、`motorbike`、`person`、`pottedplant`、`sheep`、`sofa`、`train`、`tvmonitor`。标签为 1 到 20，0 保留给背景。
